...
function mount(){if(window.__mv_trend_boot) return; window.__mv_trend_boot=true; const main=document.querySelector('main')||document.body; let host=document.getElementById('mv-trendpanel-root'); if(!host){host=document.createElement('div');host.id='mv-trendpanel-root';main.appendChild(host);} const root=(host as any).__root||(host && createRoot(host)); (host as any).__root=root; root.render(<TrendPanel ohlc={o} />); const gd=document.querySelector('.js-plotly-plot') as any; if(gd&& (window as any).Plotly) hideHighLow(gd);}

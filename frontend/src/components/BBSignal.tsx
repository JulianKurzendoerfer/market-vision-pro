...
export default function BBSignal(){
  ...
  useEffect(()=>{
    const run=()=>{
      ...
      const tr:any=data.find(t=>String(t?.name||"").toLowerCase()==="close")||
                    data.find(t=>t?.y&&t?.mode==="lines")
      if(!tr?.y) return
      const close=(tr.y as any[]).map((v:any)=>Number(v)).filter((v:any)=>isFinite(v))
      if(close.length) setSig(computeBBSignal(close))
    }
    run()
  ...
  return (
    <div style={{position:"absolute",left:8,top:8,padding:"2px 10px",fontSize:12,borderRadius:8,
                 backgroundColor:sig.bg,color:sig.fg,border:"1px solid rgba(0,0,0,0.25)",
